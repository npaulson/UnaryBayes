...
def likelihood(param, D):
    ...
    if D['At_Cp'] is not []:
        ...
        prob_Cp = ss.t.logpdf(dA_Cp, dof,
                              loc=0, scale=D['Et_Cp']/hypvec_Cp).sum()
    ...
    if D['At_H'] != []:
        ...
        prob_H = ss.t.logpdf(dA_H, dof,
                             loc=0, scale=D['Et_H']/hypvec_H).sum()
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        ...
        #                   'Ros2001', 'Wol1957']
        D['name_list_Cp'] = ['Kor2005', 'Par2003']
        D['name_list_H'] = ['Cag2008', 'Ros2001']
        ...
        if nhyp_Cp > 0:
            if nhyp_H == 0:
                hyp_Cp = np.mean(flattrace[:, -nhyp_Cp:], 0)
            else:
